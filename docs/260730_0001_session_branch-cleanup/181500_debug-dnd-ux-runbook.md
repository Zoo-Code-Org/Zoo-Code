...
line**; current main is 25 commits ahead of the merge-base `d5a8c4a3c` on a _different_ PR line
...
> commits that _reference_ "remove task-dnd contamination" — they are STATS-branch hygiene, not DND.
...
But remote `0453c3a70` **actively uses** both (multi-line import). That hunk is a _regression
artifact of the contaminated base_ — NOT a real fix. **Resolution: keep the remote (theirs during
...
During `git cherry-pick` the conflicted file is the _new_ commit applying onto remote HEAD, so:
