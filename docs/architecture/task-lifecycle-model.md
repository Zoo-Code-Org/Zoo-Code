...
| [#921](https://github.com/Zoo-Code-Org/Zoo-Code/issues/921): delegation across parallel tabs lacks coverage for different view-local mode/profile state. Follow-up [#1623](https://github.com/Zoo-Code-Org/Zoo-Code/issues/1623) proves a correct snapshot is insufficient when downstream readers use shared provider mode. | Delegation must bind an explicit immutable execution-context snapshot, and every mode-sensitive consumer must read the child task's context rather than whichever provider/view is focused.       | This checker proves the write-side selector matrix and retains a divergent task/provider-mode witness. Runtime reader correctness is intentionally owned by [PR #1625](https://github.com/Zoo-Code-Org/Zoo-Code/pull/1625), its delegated-mode reader check, and focused environment/tool tests.           |
