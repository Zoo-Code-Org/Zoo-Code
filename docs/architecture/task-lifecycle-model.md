...
Before the commit, the child's own history record is written as a durable write-ahead record. It carries a secret-free `pendingHandoff` marker (`HistoryItem.pendingHandoff`, versioned, discriminated `set`/`preserve`/`clear`) holding the requested mode and the explicit profile projection intent. The full API configuration is deliberately NOT persisted: restart re-resolves it from the durable profile store by name, exactly like normal resumed tasks. The protocol makes the delegation commit illegal until this record is durable (`wal-child` before `commit-delegation`; committing from `child-created` is rejected as `wal-required`).
