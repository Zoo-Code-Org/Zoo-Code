...
The repository-reserved `Zoo Code / PR review gate` commit status passes only after the sequence completes. It is advisory by default; repository administrators can make that exact context required on `main` if enforcement is desired. Pin the required-status rule to the GitHub Actions integration. The reserved gate context and `Zoo Code / reconcile PR review state` job name identify this workflow's own outputs; generic or externally owned contexts such as `PR review gate` or `reconcile` remain independent requirements. The labels and managed comment remain the maintainer-facing review queue either way.
