...
test("renders task organization UI with folders and pins in the VS Code dark theme", async ({ mount }) => {
	...
	await expect(component).toHaveScreenshot("task-organization-default-dark.png", { maxDiffPixels: 10000 })
})
...
test("renders expanded folder with member tasks in the VS Code dark theme", async ({ mount }) => {
	...
	await expect(component).toHaveScreenshot("task-organization-expanded-folder-dark.png", { maxDiffPixels: 10000 })
})
...
test("renders pinned tasks section in the VS Code dark theme", async ({ mount }) => {
	...
	await expect(component).toHaveScreenshot("task-organization-pinned-dark.png", { maxDiffPixels: 10000 })
})
