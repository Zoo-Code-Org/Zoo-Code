...
test("renders inline shell selector with default auto selection", async ({ mount }) => {
	...
	await expect(component).toHaveScreenshot("terminal-settings-shell-default-dark.png", { maxDiffPixels: 10000 })
})
...
test("renders effective shell info when a profile is selected", async ({ mount }) => {
	...
	await expect(component).toHaveScreenshot("terminal-settings-shell-profile-selected-dark.png", { maxDiffPixels: 10000 })
})
