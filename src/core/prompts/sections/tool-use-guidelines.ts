...
export function getToolUseGuidelinesSection(context?: SystemPromptContext): string {
	if (context && context.availableToolNames.size === 0) {
		return ""
	}

	const listFilesExample =
		isPromptToolAvailable(context, "list_files") && isPromptToolAvailable(context, "execute_command")
