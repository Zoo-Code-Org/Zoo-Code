...
describe("workspace-scoped codebase_search filtering", () => {
	it("retains search in native filtering and policy when the supplied scope is ready", () => {
		...
		expect(
			resolveEffectiveToolPolicy({ mode: "code", codeIndexManager: scope.codeIndexManager }).tools.has(
				"codebase_search",
			),
		).toBe(true)
	})
	...
	const consumers = [
		...
		{
			name: "resolveEffectiveToolPolicy",
			available: (scope?: CodeIndexWorkspaceScope) =>
				Array.from(
					resolveEffectiveToolPolicy({ mode: "code", codeIndexManager: scope?.codeIndexManager }).tools,
				),
		},
	]
	...
	it("does not let a ready workspace bypass a custom mode without the read group", () => {
		...
		expect(
			resolveEffectiveToolPolicy({
				mode: mode.slug,
				customModes: [mode],
				codeIndexManager: scope.codeIndexManager,
			}).tools.has("codebase_search"),
		).toBe(false)
	})
})
