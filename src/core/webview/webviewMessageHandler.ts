...
import type { WebviewMessageHandlerRegistry } from "./WebviewMessageHandlerRegistry"
...
export const webviewMessageHandler = async (
	...
	webviewMessageHandlerRegistry?: WebviewMessageHandlerRegistry,
) => {
	...
	switch (message.type) {
		...
		case "clearIndexData": {
			if (!webviewMessageHandlerRegistry) {
				throw new Error(`No feature handler registry configured for webview message: ${message.type}`)
			}
			await webviewMessageHandlerRegistry.handle(message)
			break
		}
