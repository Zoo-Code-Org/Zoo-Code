...
describe("ClineProvider flicker-free cancel", () => {
	...
	it("preserves delegated lineage when cancelling an already-interrupted child", async () => {
		const childHistory: HistoryItem = {
			id: "child-1",
			number: 2,
			task: "child task",
		...
		expect(mockOutputChannel.appendLine).not.toHaveBeenCalledWith(
			expect.stringContaining("Invalid task status transition: interrupted → interrupted"),
		)
	})
