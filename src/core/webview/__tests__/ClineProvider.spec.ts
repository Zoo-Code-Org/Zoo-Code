...
describe("ClineProvider", () => {
	...
	describe("browser bridge ownership", () => {
		const createFakeBridge = (port: number) => ({
			port,
			dispose: vi.fn(),
			getOrCreateVirtualWebview: vi.fn().mockReturnValue(mockWebviewView.webview),
			getPlaceholderHtml: vi.fn().mockReturnValue(`<!--bridge placeholder ${port}-->`),
			getBrowserUrl: vi.fn().mockReturnValue(`http://localhost:5173/?bridgePort=${port}`),
		})

		test("getActiveBrowserBridgePort returns undefined before the bridge is enabled", () => {
			expect(provider.getActiveBrowserBridgePort()).toBeUndefined()
		})

		test("enableBrowserBridge adopts the bridge and getActiveBrowserBridgePort reports its port", () => {
			const bridge = createFakeBridge(43210)
			provider.enableBrowserBridge(bridge as never)

			expect(provider.getActiveBrowserBridgePort()).toBe(43210)
			expect(bridge.getOrCreateVirtualWebview).toHaveBeenCalledTimes(1)
		})

		test("enableBrowserBridge ignores and disposes a second bridge (one provider -> one bridge)", () => {
			const firstBridge = createFakeBridge(43210)
			const secondBridge = createFakeBridge(43211)
			provider.enableBrowserBridge(firstBridge as never)
			provider.enableBrowserBridge(secondBridge as never)

			// The first bridge stays authoritative; the second is rejected and disposed.
			expect(provider.getActiveBrowserBridgePort()).toBe(43210)
			expect(secondBridge.dispose).toHaveBeenCalledTimes(1)
			expect(firstBridge.dispose).not.toHaveBeenCalled()
			expect(firstBridge.getOrCreateVirtualWebview).toHaveBeenCalledTimes(1)
			expect(secondBridge.getOrCreateVirtualWebview).not.toHaveBeenCalled()
		})

		test("enableBrowserBridge renders the bridge placeholder html in an already-resolved view", () => {
			const bridge = createFakeBridge(43210)
			// @ts-ignore - accessing private property for testing
			provider.view = mockWebviewView
			...
			expect(bridge.getPlaceholderHtml).toHaveBeenCalledTimes(1)
			expect(mockWebviewView.webview.html).toContain("bridge placeholder 43210")
		})

		test("resolveWebviewView renders the bridge placeholder when the bridge is active", async () => {
			const bridge = createFakeBridge(43210)
			provider.enableBrowserBridge(bridge as never)
