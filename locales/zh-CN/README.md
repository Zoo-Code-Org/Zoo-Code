...
- ✨ **新增 SOTA 模型：** 在支持的提供商中使用 GPT-6 Sol、GPT-6 Luna 和 Claude Opus 5.5。
- 🧭 **更可靠的任务和子任务：** 隔离委派模式，在重复停止后保留子任务链接，并在斜杠命令切换模式时保护编排器设置。
- 🛠️ **更可靠的终端、提供商和代码搜索行为：** 改进 Windows 和非英语环境中的终端行为，增强提供商响应与取消处理，并让代码搜索更稳定地使用正确的工作区。
