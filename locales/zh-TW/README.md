...
- ✨ **新增 SOTA 模型：** 在支援的供應商中使用 GPT-6 Sol、GPT-6 Luna 和 Claude Opus 5.5。
- 🧭 **更可靠的工作與子工作：** 隔離委派模式，在重複停止後保留子工作連結，並在斜線命令切換模式時保護協調器設定。
- 🛠️ **更可靠的終端機、供應商和程式碼搜尋行為：** 改善 Windows 和非英語環境中的終端機行為，強化供應商回應與取消處理，並讓程式碼搜尋更穩定地使用正確的工作區。
