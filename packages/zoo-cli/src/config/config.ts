...
export const Info = Schema.Struct({
	...
	experimental: Schema.optional(
		Schema.Struct({
			...
			codebase_search: Schema.optional(Schema.Boolean).annotate({
			...
			openTelemetry: Schema.Boolean.pipe(
				Schema.optional,
...
function globalConfigFile() {
	return path.join(ConfigPaths.zooGlobalConfigDir(), "zoo.jsonc")
}
...
export const layer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const loadGlobal = Effect.fnUntraced(function* () {
			...
			result = mergeConfig(result, yield* loadFile(path.join(ConfigPaths.zooGlobalConfigDir(), "zoo.jsonc")))
		...
		const [cachedGlobal, invalidateGlobal] = yield* Effect.cachedInvalidateWithTTL(
			loadGlobal().pipe(
				Effect.tapError((error) =>
					Effect.sync(() =>
						log.error("failed to load global config, using defaults", { error: String(error) }),
					),
				),
		...
		const loadInstanceState = Effect.fn("Config.loadInstanceState")(
			function* (ctx: InstanceContext) {
				...
				for (const [key, value] of Object.entries(auth)) {
					if (value.type === "wellknown") {
						...
						const next = yield* Effect.tryPromise({
							...
							Effect.tap(() =>
								Effect.sync(() => log.debug("loaded remote config from well-known", { url })),
							),
				...
				yield* merge(ConfigPaths.zooGlobalConfigDir(), global, "global")
				...
				if (!Flag.KILO_DISABLE_PROJECT_CONFIG) {
					// kilocode_change start - discover legacy config files first, then Zoo project config as highest priority.
					for (const name of ["opencode", "kilo", "zoo"] as const) {
						for (const file of yield* ConfigPaths.files(name, ctx.directory, ctx.worktree).pipe(
							Effect.orDie,
						)) {
							yield* merge(
								file,
				...
				for (const dir of unique(directories)) {
					...
					result.agent = mergeDeep(
						result.agent ?? {},
						yield* Effect.promise(() => ConfigAgent.load(dir, warnings)),
					)
					result.agent = mergeDeep(
						result.agent ?? {},
						yield* Effect.promise(() => ConfigAgent.loadMode(dir, warnings)),
					)
					// kilocode_change end
				...
				if (process.env.KILO_CONFIG_CONTENT) {
					...
					yield* merge(
						...
						yield* loadConfig(process.env.KILO_CONFIG_CONTENT, {
							...
							Effect.tap(() =>
								Effect.sync(() => log.debug("loaded custom config from KILO_CONFIG_CONTENT")),
							),
							Effect.catchDefect((err: unknown) => {
								caughtWarning(warnings, source, err)
								return Effect.succeed({} as Info)
		...
		const updateGlobal = Effect.fn("Config.updateGlobal")(function* (
			...
			if (!file.endsWith(".jsonc")) {
				...
				if (changed) yield* fs.writeWithDirs(file, serialized).pipe(Effect.orDie)
				next = merged
			} else {
				const updated = patchJsonc(before, patch)
				next = ConfigParse.effectSchema(Info, ConfigParse.jsonc(updated, file), file)
				changed = updated !== before
				if (changed) yield* fs.writeWithDirs(file, updated).pipe(Effect.orDie)
			}
