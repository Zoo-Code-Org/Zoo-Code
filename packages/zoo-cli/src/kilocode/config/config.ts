...
export namespace KilocodeConfig {
	...
	/** Zoo config file names, preferred for new project config. */
	export const ZOO_CONFIG_FILES = ["zoo.jsonc", "zoo.json"] as const
	...
	export const ALL_CONFIG_DIR_SUFFIXES = [".zoo", ".kilo", ".kilocode", ".opencode"] as const
	 ...
	 *
	 * Prefer existing Zoo root/ancestor config files and create `zoo.jsonc` when
	 * no Zoo project config exists yet. Legacy config files are read for migration
	 * but are no longer write targets.
	 */
	export const projectConfigUpdateTarget = Effect.fn("KilocodeConfig.projectConfigUpdateTarget")(function* (input: {
		...
		const zooRoot = path.join(input.directory, "zoo.jsonc")
		const zooFiles = yield* input.fs
			.up({ targets: [...ZOO_CONFIG_FILES], start: input.directory, stop: input.worktree })
			.pipe(Effect.orDie)
		return [zooRoot, ...zooFiles].find((file) => existsSync(file)) ?? zooRoot
	})
	...
	export async function loadLegacyConfigs(input: {
		...
		// Load .zooignore patterns, with legacy .kilocodeignore as a fallback
		try {
	...
	export async function migrateBashPermission() {
		const files = [
			...GLOBAL_CONFIG_FILES.map((f) => path.join(Global.Path.config, f)),
			path.join(os.homedir(), ".config", "zoo-code", "zoo.jsonc"),
		]
		...
		const target =
			existing.length > 0 ? existing[existing.length - 1] : path.join(Global.Path.config, "config.json")
