...
export namespace RulesMigrator {
	...
	export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
		...
		for (const mode of KNOWN_MODES) {
			// Mode-specific directories (.zoo/rules-{mode}/*.md, with legacy fallbacks)
			const modeSeen = new Set<string>()
			for (const prefix of [".zoo", ".kilo", ".kilocode"]) {
	...
	export async function migrate(options: {
		...
		for (const rule of rules) {
			...
			if (rule.source === "legacy") {
				warnings.push(
					`Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .zoo/rules/ directory.`,
				)
			}
		}
