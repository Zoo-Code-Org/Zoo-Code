...
export namespace ModesMigrator {
	...
	async function readZooModeFiles(projectDir: string): Promise<KilocodeMode[]> {
		const modesDir = path.join(projectDir, ".zoo", "modes")
		let entries: string[]
		try {
		...
		const modes: KilocodeMode[] = []
		for (const entry of entries.filter((name) => name.endsWith(".json")).sort()) {
			const file = path.join(modesDir, entry)
			const raw = JSON.parse(await fs.readFile(file, "utf-8"))
			const candidates = Array.isArray(raw) ? raw : Array.isArray(raw?.customModes) ? raw.customModes : [raw]
			for (const candidate of candidates) {
				const slug = candidate?.slug ?? path.basename(entry, ".json")
				modes.push({
					slug,
					name: candidate?.name ?? slug,
					roleDefinition: candidate?.roleDefinition ?? candidate?.prompt ?? "",
