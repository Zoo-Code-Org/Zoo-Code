...
export class Agent implements ACPAgent {
				...
				const next = prev
					.then(async () => {
						...
						if (res.outcome.optionId !== "reject" && permission.permission == "edit") {
							...
							const content = (await Filesystem.exists(filepath))
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethod: AuthMethod = {
			description: "Run `zoo auth login` in the terminal",
	...
	private async resolveModeState(
		...
		const currentModeId =
			...
			(await (async () => {
				...
				const defaultAgentName = await AppRuntime.runPromise(
					AgentModule.Service.use((svc) => svc.defaultAgent()),
				)
