...
export namespace KiloSessions {
	...
	export async function enableRemote() {
		...
		enabling = (async () => {
			const token = await kilocodeToken()
			if (!token) {
				throw new Error("Unable to enable remote: no Kilo credentials found. Run `zoo auth login`.")
			}

			const valid = await authValid(token)
			if (valid === false) {
				throw new Error("Unable to enable remote: invalid or expired Kilo credentials. Run `zoo auth login`.")
			}
	...
	export async function share(sessionId: string) {
		...
		const client = await getClient()
		if (!client) {
			throw new Error("Unable to share session: no Kilo credentials found. Run `zoo auth login`.")
		}
	...
	export async function unshare(sessionId: string) {
		...
		const client = await getClient()
		if (!client) {
			throw new Error("Unable to unshare session: no Kilo credentials found. Run `zoo auth login`.")
		}
