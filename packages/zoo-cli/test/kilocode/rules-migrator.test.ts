...
describe("RulesMigrator", () => {
	describe("discoverRules", () => {
		...
		test("discovers preferred .zoo/rules/ directory", async () => {
			await using tmp = await tmpdir({
				init: async (dir) => {
					await fs.mkdir(path.join(dir, ".zoo", "rules"), { recursive: true })
					await Bun.write(path.join(dir, ".zoo", "rules", "coding.md"), "# Zoo coding rules")
				},
			})
		...
		test(".zoo/rules/ takes precedence over legacy rule directories for same filename", async () => {
			await using tmp = await tmpdir({
				init: async (dir) => {
					await fs.mkdir(path.join(dir, ".zoo", "rules"), { recursive: true })
					await Bun.write(path.join(dir, ".zoo", "rules", "main.md"), "# Zoo rules")
					await fs.mkdir(path.join(dir, ".kilo", "rules"), { recursive: true })
			...
			// Only one main.md should be found (.zoo wins)
			expect(mainRules).toHaveLength(1)
			expect(mainRules[0].path).toContain(`.zoo${path.sep}`)
		})
	...
	describe("migrate", () => {
		...
		test("warns about legacy files", async () => {
			...
			expect(result.warnings.some((w) => w.includes(".zoo/rules/"))).toBe(true)
		})
