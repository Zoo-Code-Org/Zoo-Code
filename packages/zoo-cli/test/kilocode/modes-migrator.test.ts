  ...
  - slug: translate
    ...
    groups:
      ...
      - edit`,
		...
		test("loads preferred project .zoo/modes/*.json modes", async () => {
			await using tmp = await tmpdir({
				init: async (dir) => {
					await fs.mkdir(path.join(dir, ".zoo", "modes"), { recursive: true })
					await Bun.write(
						path.join(dir, ".zoo", "modes", "reviewer.json"),
						JSON.stringify({ name: "Reviewer", prompt: "Review code carefully", groups: ["read", "edit"] }),
					)
				},
			})
			...
			expect(result.agents.reviewer.prompt).toBe("Review code carefully")
